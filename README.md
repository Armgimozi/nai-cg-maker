...
- **문서 작성** — 제목·이름(slug)·태그·속성·본문(마크다운). `[[다른문서]]` 로 문서끼리 연결.
  목록은 제목·태그·속성·본문 키워드로 **검색**되고, **백업/복원**(JSON)으로 옮길 수 있습니다.
- **폴더** — 이름을 `아이템/녹슨-검` 처럼 적으면 `wiki/아이템/녹슨-검.md` 에 저장되고 목록이
  폴더별로 묶입니다(권장: `아이템/ 스킬/ 적/ 인물/ 세계관/ _템플릿/`).
- **속성** — 문서 맨 위 front matter 에 `공격력: 12` 처럼 `키: 값` 을 적으면 화면에 표로 보이고,
  웹에서 편집해도 **한 글자도 바뀌지 않고 보존**됩니다(목록·여러 줄 값·주석 포함). 게임 수치 데이터를
  여기 두면 AI 가 `/llms-full.txt` 로 긁어 표·밸런스 점검·데이터 파일 변환 등 가공을 할 수 있습니다.
