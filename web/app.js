...
async function findTags() {
  ...
  try {
    ...
    setBox("#tagStatus", "", ""); lastTags = data.tags || []; $("#ratingSeg").hidden = false; renderTags();
...
async function composeScene(scene, found) {
...
async function seqStart() {
  ...
  try {
    ...
    const p = await composeScene(seq[0], found0); seqFoundation = p;
...
async function seqNext() {
  ...
  try {
    ...
    const p = await composeScene(seq[seqIdx], found); seqFoundation = p;
