...
- 📚 **위키는 읽기가 공개**입니다(AI 가 키 없이 읽도록 의도). 편집까지 아무나 하게 두지 않으려면 호스트 환경변수 `WIKI_TOKEN` 을 설정하고, 앱의 🔑 API 키 칸 *위키 편집 토큰*에 같은 값을 넣으세요.
- 무료 호스팅은 재배포 시 디스크가 초기화됩니다. 위키 문서(`wiki/*.md`)는 **저장소에 커밋**해 두거나 위키 탭의 **백업**으로 내려받으세요. (`WIKI_DIR` 로 영구 디스크 경로를 지정할 수도 있습니다.)
